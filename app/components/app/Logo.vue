<template>
  ...
  />
  <UIcon v-else-if="isIcon" :name="src" :size="computedSize" class="shrink-0" />
  <div v-else :class="fallbackClass" :style="fallbackStyle" class="shrink-0">
...
const computedSize = computed(() => {
  const s = props.size;
  return typeof s === "string" ? parseInt(s, 10) || 32 : s;
});
...
const isImage = computed(() => {
  ...
  return (
    !!s &&
    (/^https?:\/\//.test(s) || /\.(png|jpe?g|gif|svg|webp|ico)$/i.test(s))
  );
});

const isIcon = computed(() => {
  return !!props.src && /^i-/.test(props.src);
});
...
const roundedClass = computed(
  () =>
    roundedMap[String(props.rounded) as keyof typeof roundedMap] ||
    "rounded-md",
);

const fallbackClass = computed(() => [
  "flex items-center justify-center shrink-0 bg-primary text-primary-foreground font-semibold",
  roundedClass.value,
]);
...
const fallbackStyle = computed(() => ({
  ...
  fontSize: `${computedSize.value * 0.45}px`,
}));
</script>
